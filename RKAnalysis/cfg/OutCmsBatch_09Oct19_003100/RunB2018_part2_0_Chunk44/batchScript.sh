...
do
   ...
   for try in `seq 1 3`; do
      ...
      echo "eos cp `pwd`/$f /eos/cms/store/group/cmst3/group/bpark/gkaratha/cmg_ntuples_test/RunB2018_part2_0/${ff}_44.root"
      eos cp `pwd`/$f /eos/cms/store/group/cmst3/group/bpark/gkaratha/cmg_ntuples_test/RunB2018_part2_0/${ff}_44.root
      if [ $? -ne 0 ]; then
         echo "ERROR: remote copy failed for file $ff"
         continue
      fi
      echo "remote copy succeeded"
      remsize=$(eos find --size /eos/cms/store/group/cmst3/group/bpark/gkaratha/cmg_ntuples_test/RunB2018_part2_0/${ff}_44.root | cut -d= -f3) 
      ...
      echo "everything ok"
      rm $f
      echo root://eoscms.cern.ch//eos/cms/store/group/cmst3/group/bpark/gkaratha/cmg_ntuples_test/RunB2018_part2_0/${ff}_44.root > $f.url
      break
   done
done
echo
